...
namespace concox_v3.Concox.Response
{
    public class Rp787816
    {
        ...
        { // ------ 16
            ...
            string aux = "";
            aux = Convert.ToString(data.Substring(0, 2));
            dataPacket[0] = Convert.ToString(int.Parse(aux, NumberStyles.HexNumber)); // Start Bit 1/2
            aux = Convert.ToString(data.Substring(2, 2));
            dataPacket[1] = Convert.ToString(int.Parse(aux, NumberStyles.HexNumber)); // Start Bit 2/2
            aux = Convert.ToString("05");
            dataPacket[2] = Convert.ToString(int.Parse(aux, NumberStyles.HexNumber)); // Packet Length
            aux = Convert.ToString("16");
            dataPacket[3] = Convert.ToString(int.Parse(aux, NumberStyles.HexNumber)); // Protocol Number
            aux = Convert.ToString(data.Substring(72, 2));
            dataPacket[4] = Convert.ToString(int.Parse(aux, NumberStyles.HexNumber)); // Serial Number 1/2
            aux = Convert.ToString(data.Substring(74, 2));
            ...
            string respostaPost = "";
            respostaPost += data.Substring(0, 4); // Start Bit
            respostaPost += "05"; // Packet Length
            respostaPost += "13"; // Protocol Number
            respostaPost += data.Substring(18, 4); // Serial Number
            respostaPost += crcHexadecimal; // Error check
            respostaPost += data.Substring(26, 4); // Stop Bit
